...
class CloudConvert
{
    ...
    */
    ...
    public static function createProcess($inputformat, $outputformat, $apikey = null) {
        ...
        $data = $instance->req('https://api.cloudconvert.org/process', array('inputformat' => $inputformat, 'outputformat' => $outputformat, 'apikey' => $apikey));

        if (strpos($data->url, 'http') === false) $data->url = "https:" . $data->url;
    ...
    */
    public function upload($filepath, $outputformat) {
        $this->req($this->url, array_merge(array('input' => 'upload', 'format' => $outputformat, 'file' => (class_exists('CURLFile') ? new CURLFile($filepath) : '@' . $filepath)
         // CURLFile is available PHP >= 5.5.0
        ), $this->options));
    }
    ...
    */
    public function uploadByURL($url, $filename, $outputformat) {
        $this->req($this->url, array_merge(array('input' => 'download', 'format' => $outputformat, 'filename' => $filename, 'link' => $url), $this->options));
    }
    ...
    */
    public function status($action = null) {
        if (empty($this->url)) throw new Exception("No process URL found! (Conversion not started)");
    ...
    */
    public function download($target) {
        if (empty($this->data->output->url)) throw new Exception("No download URL found! (Conversion not finished or failed)");
        if (strpos($this->data->output->url, 'http') === false) $this->data->output->url = "https:" . $this->data->output->url;
        $fp = fopen($target, 'w+');
        $ch = curl_init();
    ...
    */
    public function downloadStream() {
        ...
        $ch = curl_init();
        curl_setopt_array($ch, array(CURLOPT_FOLLOWLOCATION => true, CURLOPT_HEADER => false, CURLOPT_HTTPGET => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 30, CURLOPT_URL => $this->data->output->url));
